...
Gepaarter Code-und-DB-Rollback, etwa nach Migration 005:
...
Vor jedem Update müssen das alte Code-Release **und** der nach dem finalen Stop erzeugte Rollback-Snapshot als Paar aufbewahrt werden. Migration 005 verändert die DB-Struktur; deshalb niemals nur den Code zurückrollen.
...
4. Rollback ausschließlich mit `restore-production.sh`, dem **zum Ziel-Code-Release gepaarten** Snapshot und der expliziten Ziel-Release-ID ausführen. Das Skript übernimmt DB-Safetyset, Units, atomaren Symlink, Zustände und Health. Migration 005 verbietet einen reinen Code-Rollback.
...
Erwartet: Health HTTP 200, `sqliteReady:true`, Schema 5, `quick_check=ok`, keine Google-Health-Routen (404) und ausreichend freier Speicher. Fehlerhafte Health-Antworten enthalten keine Profildaten.

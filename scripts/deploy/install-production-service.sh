...
rollback_activation() {
  ...
  if [[ $current_target == "$candidate" ]]; then
    ...
    if [[ $had_database == true && -f $rollback_snapshot ]]; then
      ...
      elif ! runuser -u pace -- /usr/bin/node "$candidate/scripts/backup/verify-database.mjs" --allow-migrate-from 1..11 "$DATABASE_PATH"; then rollback_ok=false
      fi
    fi
...
if [[ -f $DATABASE_PATH ]]; then
  ...
  runuser -u pace -- /usr/bin/node "$candidate/scripts/backup/verify-database.mjs" --allow-migrate-from 1..11 "$rollback_snapshot"
fi

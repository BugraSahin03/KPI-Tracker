...
try {
  ...
  if (health.status !== 'ok' || health.sqliteReady !== true || health.schemaVersion !== 12) throw new Error('Unerwartete Health-Antwort.')
  ...
  if (verification.prepare('SELECT MAX(version) AS version FROM schema_migrations').get().version !== 12) throw new Error('Backup hat unerwartetes Schema.')
  verification.close()

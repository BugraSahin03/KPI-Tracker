...
describe('Production-Datenbankverifier', () => {
  it('akzeptiert ausschließlich eine vollständige Schema-5-Pace-Datenbank', async () => {
    const filename = databaseFile()
    await expect(execFileAsync(process.execPath, [verifier, filename])).resolves.toMatchObject({ stdout: expect.stringContaining('Schema 5') })
  })
  ...
  it('lehnt zu neue Schemas, fehlende Profile/Tabellen und Fremdschlüsselfehler ab', async () => {
    for (const corruption of ['schema', 'profile', 'table', 'foreign-key'] as const) {
      ...
      if (corruption === 'schema') database.prepare('INSERT INTO schema_migrations(version,applied_at) VALUES (6,?)').run(new Date().toISOString())
  ...
  it('führt den Migrationsmodus ausdrücklich nur auf einer temporären Kopie aus', () => {
    ...
    expect(source).toContain("argumentsList[1] !== '1..5'")
